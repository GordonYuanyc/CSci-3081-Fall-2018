...
void PixelBuffer::SaveToFile(const std::string &filename) {
  ...
  imageio::Image* image_ = new imageio::Image(x, y, 4);
  for (int i = 0; i < x; i++) {
    for (int j = 0; j < y; j++) {
      ColorData temp = this->pixel(i, j);
      image_->SetFloatValue(i, j, 0, temp.red());
      image_->SetFloatValue(i, j, 1, temp.green());
      image_->SetFloatValue(i, j, 2, temp.blue());
      image_->SetFloatValue(i, j, 3, temp.alpha());
    }
  }
  imageio::ImageManager::instance().SaveToFile(filename, *image_);
  delete image_;
}

void PixelBuffer::LoadFromFile(const std::string &filename) {
  imageio::Image* image_ = imageio::ImageManager::instance().LoadFromFile(
    filename);
  this->Resize(image_->Width(), image_->Height());
  for (int i = 0; i < image_->Width(); i++) {
    for (int j = 0; j < image_->Height(); j++) {
      ColorData temp;
      temp.set_red(image_->FloatValue(i, j, 0));
      temp.set_green(image_->FloatValue(i, j, 1));
      temp.set_blue(image_->FloatValue(i, j, 2));
      temp.set_alpha(image_->FloatValue(i, j, 3));
      this->set_pixel(i, j, temp);
    }
  }
  delete image_;
  (void)filename;
